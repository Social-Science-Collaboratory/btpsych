...
df <- df %>%
  ...
  distinct(clean_title, .keep_all = T) %>%
  # extract the first author's name from the author dataframe
  mutate(
    first_author = map_chr(author, ~ .x$au_display_name[[1]]),
...
identify_duplicates <- function(df, minhash) {
  ...
  buckets <- lsh(corpus, bands = 50, progress = interactive())
...
df_list <- lapply(
  1:n_batch,
  function(x, batch_size = ceiling(nrow(df) / n_batch)) {
...
similarities <- readRDS(file.path(
  data_dir,
  "deduplication",
  "similarities_10batches_5%sim.Rds"
))
...
similarities_validation <- read_csv(file.path(
  data_dir,
  "deduplication",
  "similarities_validation_10batches_J.csv"
))
...
saveRDS(remove_list, file.path(data_dir, "deduplication", "remove_list.Rds"))
